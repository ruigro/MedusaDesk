<p align="center">
  <img src="res/logo-header.svg" alt="Medusa Desk - Far Island Corporation remote desktop"><br>
...
- **v0.1.8:** Built-in AI Agent Gateway for headless remote control, a signed Windows installer, Linux x86_64 and ARMv7 `.deb` packages, automatic update checks, and reliable clean builds without machine-specific native dependency paths.
...
| Version | Highlights |
| --- | --- |
| v0.1.8 | AI Agent Gateway, signed Windows installer, Linux x86_64/ARMv7 `.deb` packages, update checks, and portable clean-build dependency handling |
...
Medusa Desk ships a built-in gateway that lets AI agents (Claude Code, Cursor,
scripts over SSH) control your saved peers headlessly — screenshots, input,
shell commands and file transfer — through three interfaces sharing one core:

```sh
medusadesk agent screenshot --peer <ID> --out shot.png   # one-shot CLI (great over SSH)
claude mcp add medusadesk -- medusadesk agent mcp        # MCP server for AI coding agents
medusadesk agent serve --port 21120                      # localhost HTTP JSON API
```

Agents authenticate with the same peer passwords/2FA as human users. See
[docs/AI_AGENT_GATEWAY.md](docs/AI_AGENT_GATEWAY.md) for the full reference,
and the in-app **Settings → AI Agents** page for setup snippets.

Build Medusa Desk from this source tree before distributing branded binaries.
