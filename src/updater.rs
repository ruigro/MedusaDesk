...
fn check_update(manually: bool) -> ResultType<()> {
    ...
    let update_msi = crate::platform::is_msi_installed()?
        && (!crate::is_custom_client() || crate::common::is_medusadesk());
    ...
    if update_url.is_empty() {
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            let download_url = if crate::common::is_medusadesk() {
                let download_file = crate::common::SOFTWARE_UPDATE_DOWNLOAD_FILE
                    .lock()
                    .unwrap()
                    .clone();
                if download_file.is_empty() {
                    bail!("No MedusaDesk update asset selected for this platform.");
                }
                format!("{}/{}", download_url, download_file)
        ...
        {
            log::debug!("New version available: {}", update_url);
        }
    }
    Ok(())
}
