...
pub fn core_main() -> Option<Vec<String>> {
    ...
    // Headless AI agent gateway: `medusadesk agent <verb> ...`.
    // Runs entirely without a UI and never falls through to the GUI startup.
    #[cfg(feature = "agent")]
    if args.first().map(|a| a == "agent").unwrap_or(false) {
        crate::agent::cli::run(args[1..].to_vec());
        return None; // unreachable: run() exits the process
    }
    ...
    if args.is_empty() || crate::common::is_empty_uni_link(&args[0]) {
        ...
        // Settings -> AI Agents: auto-start the localhost HTTP gateway.
        #[cfg(feature = "agent")]
        crate::agent::start_http_gateway_if_enabled();
    } else {
