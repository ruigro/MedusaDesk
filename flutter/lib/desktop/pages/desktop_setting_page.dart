...
class DesktopSettingPage extends StatefulWidget {
  ...
  static final List<SettingsTabKey> tabKeys = [
    ...
    if (!isWeb && !bind.isDisableSettings()) SettingsTabKey.aiAgents,
    ...
    for (final tab in DesktopSettingPage.tabKeys) {
      switch (tab) {
        ...
        case SettingsTabKey.printer:
          ...
          break;
        case SettingsTabKey.aiAgents:
          settingTabs.add(_TabInfo(
              tab, 'AI Agents', Icons.smart_toy_outlined, Icons.smart_toy));
          break;
...
class _AboutState extends State<_About> {
  ...
  Widget build(BuildContext context) {
    return futureBuilder(future: () async {
      ...
      return SingleChildScrollView(
        controller: scrollController,
        child: _Card(title: 'About Medusa Desk', children: [
          Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              const SizedBox(
              ...
              if (!isWeb)
                SelectionArea(
                    child: Text('${translate('Fingerprint')}: $fingerprint')
                        .marginSymmetric(vertical: 4.0)),
              Text('Far Island Corporation').marginSymmetric(vertical: 4.0),
              Container(
                  ...
                  children: [
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text(
                            'Medusa Desk by Far Island Corporation\n$license',
...
class _AiAgents extends StatefulWidget {
  const _AiAgents({Key? key}) : super(key: key);

  @override
  State<_AiAgents> createState() => _AiAgentsState();
}

class _AiAgentsState extends State<_AiAgents>
    with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;

  final scrollController = ScrollController();
  final RxString _port = ''.obs;

  @override
  void initState() {
    super.initState();
    final p = bind.mainGetOptionSync(key: kOptionAgentHttpPort);
    _port.value = p.isEmpty ? kDefaultAgentHttpPort : p;
  }
  ...
  Widget _snippet(BuildContext context, String snippet) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    return Container(
      margin: EdgeInsets.only(left: _kContentHMargin, top: 8),
          ...
          Expanded(
            child: SelectableText(
              ...
              style: TextStyle(
                ...
                fontSize: 12.5,
                color: isDark ? MedusaColors.biolume : MedusaColors.biolumeDim,
              ),
            ),
          ),
          InkWell(
            ...
            child: Tooltip(
              ...
              child: Icon(Icons.copy_rounded,
                  size: 16, color: MedusaColors.textMuted),
            ),
          ).marginOnly(left: 8),
        ],
      ),
  ...
  Widget gateway(BuildContext context) {
    ...
    return _Card(title: 'AI Agent Gateway', children: [
      ...
      _OptionCheckBox(
        context,
        'Enable HTTP gateway (localhost only)',
        kOptionAgentHttpEnabled,
        update: (_) => setState(() {}),
        optGetter: () =>
            bind.mainGetOptionSync(key: kOptionAgentHttpEnabled) == 'Y',
        optSetter: (k, v) async =>
          ...
          SizedBox(
            ...
            child: TextField(
              ...
              decoration: InputDecoration(
                hintText: kDefaultAgentHttpPort,
                contentPadding:
                    EdgeInsets.symmetric(vertical: 12, horizontal: 12),
              ),
            ).workaroundFreezeLinuxMint().marginOnly(right: 15),
          ...
          Obx(() => ElevatedButton(
                onPressed: applyEnabled.value
                    ? () async {
                        applyEnabled.value = false;
                        await bind.mainSetOption(
                            key: kOptionAgentHttpPort, value: controller.text);
                        _port.value = controller.text.isEmpty
                            ? kDefaultAgentHttpPort
                            : controller.text;
                      }
                    : null,
  ...
  Widget mcpSetup(BuildContext context) {
    return _Card(title: 'MCP Setup', children: [
      _hint(context,
  ...
  Widget recentSessions(BuildContext context) {
    return _Card(
      title: 'Recent Agent Sessions',
      title_suffix: [
        IconButton(
          icon: Icon(Icons.refresh, size: 18, color: MedusaColors.textMuted),
          onPressed: () => setState(() {}),
        ).marginOnly(right: _kContentHMargin),
      ],
      children: [
        FutureBuilder<String>(
          future: bind.mainGetOption(key: kOptionAgentSessions),
          builder: (context, snapshot) {
            List<Widget> rows = [];
            if (snapshot.hasData && snapshot.data!.isNotEmpty) {
              try {
                final list = jsonDecode(snapshot.data!) as List<dynamic>;
                rows = list
            ...
            if (rows.isEmpty) {
              return _hint(context, 'No recent agent sessions');
            }
